...
export const riskSchema = z.object({
  ...
  details: z.object({ section_title: z.string().default(''), items: z.array(z.object({ label: z.string().default(''), value: z.string().default('') }).passthrough()).default([]), underlying_exposure: strings, impact: strings }).default({ section_title: '', items: [], underlying_exposure: [], impact: [] }),
...
export function normalizeRisk(input: unknown): Risk {
  ...
  const parsed = riskSchema.parse({ ...source, sender: { name: text(oldSender.name) || 'StratSync Risk Monitor', source: text(oldSender.source) || text(oldSender.context), risk_id: text(oldSender.risk_id) || text(source.risk_id), timestamp: text(oldSender.timestamp) || text(source.detected_time), context: text(oldSender.context) }, entity: { type: text(oldEntity.type) || (entityId ? 'sku' : ''), id: entityId, name: entityName, secondary: oldEntity.secondary }, details: { section_title: text(oldDetails.section_title) || text(rec(oldSections[0]).title), items, underlying_exposure: textArray(oldDetails.underlying_exposure).length ? textArray(oldDetails.underlying_exposure) : sectionItems, impact: textArray(oldDetails.impact).length ? textArray(oldDetails.impact) : textArray(source.impact), sections: oldSections }, mitigation: { summary: text(mitigationRecord.summary), steps: stepsSource.map((v, i) => { const r = rec(v); return { ...r, step: i + 1, title: text(r.title), description: text(r.description), owner: text(r.owner) } }), last_updated: text(mitigationRecord.last_updated) || text(source.updated_at), next_action: text(mitigationRecord.next_action) } }) as unknown as Risk
  return { ...structuredClone(sampleRisk), ...parsed, sender: { ...sampleRisk.sender, ...parsed.sender }, entity: { ...sampleRisk.entity, ...parsed.entity }, details: { ...sampleRisk.details, ...parsed.details }, mitigation: decorateMitigation({ ...sampleRisk.mitigation, ...parsed.mitigation }) } as Risk
}
